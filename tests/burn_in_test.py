    ...
    def print_summary(self):
        ...
        for pattern, counts in self.pattern_counts.items():
            if counts["total"] > 0:
                ...
                print(
                    f"  {pattern:12s}: {counts['success']:4d}/{counts['total']:4d} ({rate:5.1f}%)"
                )
        print("=" * 70)
...
class BurnInTest:
    ...
    def __init__(
        self, duration_hours: float, target_queries: int, resource_sample_interval: int = 60
    ):
...
def main():
    ...
    parser.add_argument(
        "--target-queries", type=int, default=3000, help="Target number of queries (default: 3000)"
    )
    parser.add_argument(
        "--resource-interval",
        type=int,
