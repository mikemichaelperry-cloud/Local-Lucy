    ...
    def predict(self, query: str) -> dict[str, Any]:
        ...
        return {
            ...
            "probs": {
                "intent": {
                    label: round(p, 4)
                    for label, p in zip(self.intent_classes, probs["intent"].tolist())
                },
                "evidence": {
                    label: round(p, 4)
                    for label, p in zip(self.evidence_classes, probs["evidence"].tolist())
                },
                "route": {
                    label: round(p, 4)
                    for label, p in zip(self.route_classes, probs["route"].tolist())
                },
                "policy": {
                    label: round(p, 4)
                    for label, p in zip(self.policy_classes, probs["policy"].tolist())
                },
            },
        }
