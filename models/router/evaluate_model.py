...
def evaluate(checkpoint_dir: str | Path, config_path: str | Path = "config.yaml"):
    ...
    for head in ["intent", "evidence", "route", "policy"]:
        classes = {"intent": intent_classes, "evidence": evidence_classes, 
                   "route": route_classes, "policy": policy_classes}[head]
        acc = accuracy_score(y_true[head], y_pred[head])
        print(f"\n{head.upper()} — Accuracy: {acc:.4f}")
        print(classification_report(y_true[head], y_pred[head], target_names=classes, zero_division=0))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default="checkpoints/best_model", help="Checkpoint directory")
