...
SLOT_VALUES: dict[str, list[str]] = {
    ...
    "math_problem": [
        ...
        "the factorial of 6",
        "the greatest common divisor of 48 and 18",
    ],
    # Practical / life
    "life_decision": [
    ...
    "project_topic": [
        "a mobile app for mental health",
...
def generate_synthetic_examples(intent_family: str, count: int) -> list[dict]:
    ...
    for _ in range(count):
        ...
        evidence = (
            "required"
            if intent_family in ("medical_inquiry", "current_evidence")
            else "not_required"
        )
        policy = "none"
        ...
        return {
            ...
            "labels": {
                ...
                "evidence": torch.tensor(
                    self.evidence2idx[labels["evidence_mode"]], dtype=torch.long
                ),
                "route": torch.tensor(self.route2idx[labels["route"]], dtype=torch.long),
                "policy": torch.tensor(
                    self.policy2idx[labels["policy_override"]], dtype=torch.long
                ),
            },
        }
