...
class LatencyProfiler:
    """
    Latency profiler for tracking execution timing.
    
    Replaces shell latprof_now_ms() and latprof_append() functions
    with a Python-native implementation.
    """
    ...
    def start(self, stage: str) -> None:
        ...
        self.start_times[stage] = time.time_ns() // 1_000_000  # ms
    
    def end(self, stage: str) -> int:
        """End timing a stage and return elapsed ms."""
        if stage in self.start_times:
            elapsed = (time.time_ns() // 1_000_000) - self.start_times[stage]
            self.timings[stage] = elapsed
            logging.info(f"LATPROF: {stage} = {elapsed}ms")
            return elapsed
        return 0
...
them according to the route type (bypass, provisional, or full). It handles:
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        # =========================================================================
        # LATENCY PROFILER INITIALIZATION
        # =========================================================================
    ...
    async def execute_async(
        ...
        self._profiler.start("total_execution")
        
        # Check for medical context and configure safety constraints
        self._profiler.start("medical_check")
        requires_evidence, evidence_reason = requires_evidence_mode(question, context)
        self._profiler.end("medical_check")
        ...
        try:
            ...
            self._profiler.start("route_determination")
            route_type = self._determine_route_type(route, intent, context)
            self._profiler.end("route_determination")
            
            # Use Python-native execution based on route type
            self._profiler.start("route_execution")
            ...
            self._profiler.end("route_execution")
    ...
    async def _execute_full_route_python(
        ...
        if route.route in ("EVIDENCE", "NEWS", "FULL", "AUGMENTED", "TIME"):
            self._profiler.start("evidence_fetch")
            evidence = await self._fetch_evidence(question, route)
            self._profiler.end("evidence_fetch")
        ...
        self._profiler.start("prompt_build")
        prompt = self._build_augmented_prompt(question, evidence, route)
        self._profiler.end("prompt_build")
        
        # Step 3: Call appropriate provider
        self._profiler.start("provider_call")
        if route.provider == "local":
            ...
            response = await self._call_local_model_async(prompt, context)
        self._profiler.end("provider_call")
    ...
    def _format_response(
        ...
        # Strip validation markers (from extract_validated.py)
        text = text.replace("BEGIN_VALIDATED", "").replace("END_VALIDATED", "")
        text = text.replace("BEGIN_VALIDATED_CONTEXT", "").replace("END_VALIDATED_CONTEXT", "")
        
        # Strip source markers
        text = text.replace("BEGIN_SOURCES", "").replace("END_SOURCES", "")
        
        # Add context footer for unverified sources
        trust_class = metadata.get("trust_class", "")
        if trust_class == "unverified":
            provider = metadata.get("provider", "unknown")
            text += f"\n\n[Source: {provider} - verify information independently]"
