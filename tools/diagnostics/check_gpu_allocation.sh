...
if [ -n "$KOKORO_PID" ]; then
    ...
    SOCKET="${LUCY_VOICE_KOKORO_SOCKET:-/home/mike/lucy-v9/snapshots/opt-experimental-v9-dev/tmp/run/kokoro_tts_worker.sock}"
    if [ -S "$SOCKET" ]; then
...
echo "=== PYTORCH CUDA ==="
VENV_PYTHON="/home/mike/lucy-v9/ui-v9/.venv/bin/python"
