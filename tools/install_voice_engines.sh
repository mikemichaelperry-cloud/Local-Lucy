...
UI_ROOT="${LUCY_UI_ROOT:-${WORKSPACE_ROOT}/lucy-v10/ui-v9}"
...
install_ui_v8_kokoro(){
  if [[ ! -x "${UI_VENV_PY}" ]]; then
    warn "ui-v9 python not found at ${UI_VENV_PY}; skipping kokoro install"
    return 1
  fi
  if [[ ! -f "${UI_TTS_REQUIREMENTS_FILE}" ]]; then
    warn "ui-v9 TTS requirements file missing: ${UI_TTS_REQUIREMENTS_FILE}"
    return 1
  fi

  say "INFO: installing ui-v9 Kokoro/TTS requirements from ${UI_TTS_REQUIREMENTS_FILE}"
  if ! "${UI_VENV_PY}" -m pip install --upgrade -r "${UI_TTS_REQUIREMENTS_FILE}"; then
    warn "ui-v9 Kokoro/TTS install failed"
    return 1
  fi
  say "OK: ui-v9 Kokoro/TTS requirements installed"
  return 0
}
...
main(){
  ...
  install_ui_v8_kokoro || warn "ui-v9 Kokoro/TTS not installed; active v8 may fall back from kokoro"
