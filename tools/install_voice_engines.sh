...
UI_ROOT="${LUCY_UI_ROOT:-${WORKSPACE_ROOT}/lucy-v10/ui-v10}"
...
install_ui_v8_kokoro(){
  ...
  if [[ ! -f "${UI_TTS_REQUIREMENTS_FILE}" ]]; then
    warn "ui-v10 TTS requirements file missing: ${UI_TTS_REQUIREMENTS_FILE}"
    return 1
  fi

  say "INFO: installing ui-v10 Kokoro/TTS requirements from ${UI_TTS_REQUIREMENTS_FILE}"
  if ! "${UI_VENV_PY}" -m pip install --upgrade -r "${UI_TTS_REQUIREMENTS_FILE}"; then
    warn "ui-v10 Kokoro/TTS install failed"
    return 1
  fi
  say "OK: ui-v10 Kokoro/TTS requirements installed"
  return 0
}
