...
run_chat_once(){
  ...
  if [[ "${VOICE_SESSION_MEMORY}" == "1" ]]; then
    printf '%s\n/exit\n' "${routed}" | LUCY_SURFACE="voice" LUCY_ROUTE_CONTROL_MODE="${route_ctl}" LUCY_CONVERSATION_MODE_FORCE="${VOICE_CONVERSATION_FORCE}" LUCY_NL_MEMORY_FILE="${VOICE_SESSION_MEMORY_FILE}" "${NL_CHAT_BIN}" 2>&1
  else
