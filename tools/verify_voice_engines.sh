...
if [[ ! -x "${UI_VENV_PY}" ]]; then
  die "ui-v9 python missing (${UI_VENV_PY})"
fi

if ! "${UI_VENV_PY}" -c 'import kokoro, soundfile' >/dev/null 2>&1; then
  die "ui-v9 kokoro stack missing (expected modules: kokoro, soundfile)"
fi
ok "ui-v9 kokoro modules present"
