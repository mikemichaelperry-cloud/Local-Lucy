...
if [[ ! -x "${UI_VENV_PY}" ]]; then
  die "ui-v10 python missing (${UI_VENV_PY})"
fi

if ! "${UI_VENV_PY}" -c 'import kokoro, soundfile' >/dev/null 2>&1; then
  die "ui-v10 kokoro stack missing (expected modules: kokoro, soundfile)"
fi
ok "ui-v10 kokoro modules present"
