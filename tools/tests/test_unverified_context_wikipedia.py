...
def _direct_summary_title(url: str) -> str | None:
...
def _search_query(url: str) -> str | None:
    ...
    if parsed.path == "/w/api.php":
        ...
        return qs.get("srsearch", [None])[0]
    return None


def _make_http_mock(
    direct_payloads: dict[str, dict | None],
    search_results: dict[str, list[dict]],
):
    """Return a fake _http_json backed by *direct_payloads* and *search_results*."""

    def fake_http(url: str):
        title = _direct_summary_title(url)
        if title is not None:
            payload = direct_payloads.get(title)
            if payload is None:
                raise RuntimeError(f"direct summary not found: {title}")
            return payload

        query = _search_query(url)
        if query is not None:
            return {"query": {"search": search_results.get(query, [])}}
...
def test_japan_tourist_attractions_falls_back_to_tourism_in_japan(monkeypatch, tmp_path):
    ...
    direct = {
        "main tourist attractions in Japan": None,
        "Tourism in Japan": {
    ...
    search = {
        "main tourist attractions in Japan": [{"title": "Tourism in China"}],
    }
