...
class RuntimeToggleTester:
    ...
    def test_toggle(self, name: str, field: str, values: list[str], check_env: bool = True) -> dict:
        ...
        for value in values:
            try:
                ...
                print(f"  ✗ Error: {e}")
                results["tests"].append({"value": value, "error": str(e), "success": False})

        return results

    def test_mode_toggle(self) -> dict:
        """Test mode toggle (auto/online/offline)."""
        return self.test_toggle("Mode Toggle", "mode", ["auto", "online", "offline", "auto"])

    def test_conversation_toggle(self) -> dict:
        """Test conversation toggle (on/off)."""
        return self.test_toggle("Conversation Toggle", "conversation", ["on", "off", "on"])

    def test_memory_toggle(self) -> dict:
        """Test memory toggle (on/off)."""
        return self.test_toggle("Memory Toggle", "memory", ["on", "off", "off", "on"])

    def test_evidence_toggle(self) -> dict:
        """Test evidence toggle (on/off)."""
        return self.test_toggle("Evidence Toggle", "evidence", ["on", "off", "on"])

    def test_voice_toggle(self) -> dict:
        """Test voice toggle (on/off)."""
        return self.test_toggle("Voice Toggle", "voice", ["on", "off", "on"])
    ...
    def test_augmented_provider(self) -> dict:
        ...
        return self.test_toggle(
            "Augmented Provider", "augmented_provider", ["wikipedia", "grok", "openai", "wikipedia"]
        )
    ...
    def test_interaction_evidence_augmentation(self) -> dict:
        ...
        for evidence, augmentation in combinations:
            try:
                ...
                success = (
                    state.get("evidence") == evidence
                    and state.get("augmentation_policy") == augmentation
                )

                if success:
                    print(
                        f"  ✓ evidence={state['evidence']}, augmentation={state['augmentation_policy']}"
                    )
                else:
                    print(
                        f"  ✗ Mismatch: got evidence={state.get('evidence')}, augmentation={state.get('augmentation_policy')}"
                    )

                results["tests"].append(
                    {"evidence": evidence, "augmentation": augmentation, "success": success}
                )
    ...
    def test_interaction_voice_mode(self) -> dict:
        ...
        for voice, mode in combinations:
            try:
                ...
                success = state.get("voice") == voice and state.get("mode") == mode
                ...
                results["tests"].append({"voice": voice, "mode": mode, "success": success})

            except Exception as e:
                print(f"  ✗ Error: {e}")
                results["tests"].append(
                    {"voice": voice, "mode": mode, "error": str(e), "success": False}
                )
    ...
    def run_all_tests(self) -> dict:
        ...
        passed_tests = sum(
            sum(1 for t in r["tests"] if t.get("success", False)) for r in all_results
        )
