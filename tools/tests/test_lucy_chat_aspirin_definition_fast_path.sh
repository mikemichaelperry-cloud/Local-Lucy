...
out="$(LUCY_ROOT="${FAKE_ROOT}" LUCY_ROUTER_BYPASS=1 LUCY_CHAT_FORCE_MODE=EVIDENCE "${LUCY_CHAT}" "What is aspirin?")"
printf '%s\n' "${out}" | grep -q "Aspirin is a salicylate medicine" || die "missing aspirin definition answer"
...
ok "aspirin definition query uses deterministic medical evidence fast-path"

echo "PASS: test_lucy_chat_aspirin_definition_fast_path"
