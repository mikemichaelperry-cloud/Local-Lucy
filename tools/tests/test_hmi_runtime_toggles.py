...
class HMIToggleTests:
    ...
    def test_evidence_toggle(self):
        ...
        self.assert_eq(
            env.get("LUCY_ENABLE_INTERNET"), "1", "LUCY_ENABLE_INTERNET=1 (mirrors evidence)"
        )
    ...
    def test_voice_toggle(self):
        ...
        if voice_runtime_file.exists():
            ...
            log(
                f"  ℹ Voice runtime status: {voice_runtime.get('status', 'unknown')}", Colors.YELLOW
            )
    ...
    def test_augmentation_policy(self):
        ...
        for policy in policies:
            ...
            state = read_state()
            self.assert_eq(
                state["augmentation_policy"],
                policy,
                f"State file shows augmentation_policy={policy}",
            )
    ...
    def test_evidence_augmentation_interaction(self):
        ...
        for evidence, policy, description in test_cases:
            ...
            success &= self.assert_eq(
                state["augmentation_policy"], policy, f"  augmentation_policy={policy}"
            )
...
def main():
    ...
    try:
        if test_name == "all":
            ...
            log(
                "Available tests: all, evidence, voice, mode, memory, conversation, augmentation, interactions"
            )
            return 1
