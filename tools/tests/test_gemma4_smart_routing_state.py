...
def test_cli_set_model_accepts_gemma4(isolated_runtime):
    """CLI set-model must accept the local-lucy-gemma4 persona model."""
    namespace, env = isolated_runtime
    _ensure_state(namespace, env)

    result = _run_cli(env, "set-model", "--value", "local-lucy-gemma4")
    assert result.returncode == 0, result.stderr

    state = json.loads(_state_file(namespace).read_text(encoding="utf-8"))
    assert state["model"] == "local-lucy-gemma4"
    assert state["active_model"] == "local-lucy-gemma4"
