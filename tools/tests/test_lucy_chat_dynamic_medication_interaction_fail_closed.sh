...
case "${url}" in
  https://medlineplus.gov/search/\?query=lisinopril)
    ...
    printf 'Trusted medlineplus result page for lisinopril.\n'
    ;;
  https://dailymed.nlm.nih.gov/dailymed/search.cfm\?query=lisinopril)
    echo "FETCH_META status=200" >&2
    cat <<'EOF'
SEARCH RESULTS for lisinopril
DailyMed search page for lisinopril.
EOF
    ;;
  https://pubmed.ncbi.nlm.nih.gov/\?term=lisinopril)
    echo "FETCH_META status=200" >&2
    printf 'Review article on antihypertensive therapy and interaction monitoring.\n'
    ;;
  *)
...
printf '%s\n' "${out}" | grep -q "ask a clinician or pharmacist to check the exact combination" || die "missing interaction action hint"
...
[[ "$(read_field MEDICAL_STRUCTURED_STATUS)" == "insufficient" ]] || die "expected MEDICAL_STRUCTURED_STATUS=insufficient"
[[ "$(read_field MEDICAL_STRUCTURED_QUERY_FAMILY)" == "interactions" ]] || die "expected MEDICAL_STRUCTURED_QUERY_FAMILY=interactions"
[[ "$(read_field MEDICAL_STRUCTURED_COUNTERPART)" == "grapefruit" ]] || die "expected MEDICAL_STRUCTURED_COUNTERPART=grapefruit"

ok "bounded extractor fails closed for unsupported interaction facts without compose fallback"
echo "PASS: test_lucy_chat_dynamic_medication_interaction_fail_closed"
