...

{
  ...
  echo "UNVERIFIED_CONTEXT_CLASS=openai_general"
...
printf '%s\n' "${out}" | grep -q 'Augmented provider: openai' || die "missing openai provider metadata in launcher output"
printf '%s\n' "${out}" | grep -q 'Trust class: unverified' || die "missing unverified trust class in launcher output"

ok "launcher surfaces openai provider metadata for augmented unverified outcomes"
echo "PASS: test_launcher_openai_provider_metadata_visibility"
