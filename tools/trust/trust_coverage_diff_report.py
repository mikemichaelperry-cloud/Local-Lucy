...
def read_baseline_tsv(path):
    ...
    for i, line in enumerate(open(path, encoding="utf-8", errors="ignore")):
        if i == 0 and line.lower().startswith("domain\t"):
            continue
        parts = line.rstrip("\n").split("\t")
        if len(parts) < 4:
            continue
...
def parse_catalog(path):
    ...
    def ps(s):
        ...
        if s.startswith("[") and s.endswith("]"):
            inner = s[1:-1].strip()
            return (
                [] if not inner else [x.strip().strip('"') for x in inner.split(",") if x.strip()]
            )
        return s
    ...
    for line in open(path, encoding="utf-8"):
        ...
        if ind == 4 and ":" in s and cur:
            k, v = s.split(":", 1)
            data[cur][k.strip()] = ps(v)
    return data


def norm_baseline_cats(cats):
    m = {
        "news_official": "news_world",
        "news_reporting": "news_world",
        "news_business": "finance",
        "news_israel": "news_israel",
        "news_australia": "news_world",
        "knowledge": "engineering",
        "knowledge_secondary": "engineering",
        "medical_index": "medical",
        "medical": "medical",
        "medical_israel": "medical",
        "medical_australia": "medical",
        "engineering": "engineering",
        "engineering_semiconductor": "electronics",
        "engineering_australia": "engineering",
        "standards": "engineering",
        "ai_labs": "ai",
        "ai_research": "ai",
        "ai_engineering": "ai",
        "economics_data": "finance",
        "economics_israel": "finance",
        "economics_us": "finance",
        "economics_australia": "finance",
    }
    out = set()
    for c in cats:
        out.add(m.get(c, c))
...
baseline_domains = latest(os.path.join(notes, "baseline_domains_*_*.txt")) or latest(
    os.path.join(notes, "baseline_domains_*.txt")
)
...
if not os.path.isfile(cur_allow):
    fail(f"missing current allowlist: {cur_allow}")
if not os.path.isfile(cat_path):
    fail(f"missing trust catalog: {cat_path}")
if not baseline_domains or not baseline_tier:
    fail("missing phase0 baselines in dev_notes/unified_trust_layer_v1")
...
for p in (exc_removed, exc_retier, exc_cat):
    ensure_file(p)
...
report = {
    ...
    "counts": {
        "baseline_domains": len(base_domains),
        "current_domains": len(cur_domains),
        "added": len(added),
        "removed": len(removed),
        "retier": len(retier),
        "category_shrink": len(cat_shrink),
    },
    "added": added,
    "removed": removed,
...
if bad_removed or bad_retier or bad_cat:
    msgs = []
    if bad_removed:
        msgs.append(f"removed domains without exception: {len(bad_removed)}")
    if bad_retier:
        msgs.append(f"retier changes without exception: {len(bad_retier)}")
    if bad_cat:
        msgs.append(f"category shrink without exception: {len(bad_cat)}")
    fail("; ".join(msgs))
