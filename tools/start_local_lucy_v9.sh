...
if [[ "$(basename -- "${ROOT}")" != "lucy-v10" && "$(basename -- "${ROOT}")" != "opt-experimental-v9-dev" ]]; then
  echo "ERR: active v8 terminal authority requires lucy-v10 or opt-experimental-v9-dev root, got: ${ROOT}" >&2
  exit 2
fi
cd "$ROOT"
