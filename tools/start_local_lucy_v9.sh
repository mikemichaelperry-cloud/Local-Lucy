...
if [[ "$(basename -- "${ROOT}")" != "lucy-v10" ]]; then
  echo "ERR: active terminal authority requires lucy-v10 root, got: ${ROOT}" >&2
  exit 2
fi
cd "$ROOT"
...
elif [[ "$(basename -- "${ROOT}")" == "lucy-v10" ]]; then
  UI_ROOT="${ROOT}/ui-v10"
else
  WORKSPACE_ROOT="$(dirname -- "$(dirname -- "${ROOT}")")"
  UI_ROOT="${WORKSPACE_ROOT}/ui-v10"
fi
UI_ROOT="$(CDPATH= cd -- "${UI_ROOT}" 2>/dev/null && pwd)" || {
  echo "ERR: invalid LUCY_UI_ROOT/UI_ROOT: ${UI_ROOT}" >&2
  exit 2
}
if [[ "$(basename -- "${UI_ROOT}")" != "ui-v10" ]]; then
  echo "ERR: active v8 terminal authority requires ui-v10 root, got: ${UI_ROOT}" >&2
  exit 2
fi
...
export LUCY_VOICE_MODEL="${LUCY_VOICE_MODEL:-small.en}"
...
export LUCY_ROUTER_LOG_DIR="${LUCY_ROUTER_LOG_DIR:-${ROOT}/state/router_logs}"
