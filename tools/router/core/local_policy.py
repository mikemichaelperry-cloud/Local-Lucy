...
def match_local_response_id(question: str, intent: str = "") -> Optional[str]:
    ...
    if re.search(
        r"(^|[^a-z0-9_])(who am i|who i am|do you know who i am|who is (mike|michael))([^a-z0-9_]|$)",
        qn,
    ):
    ...
    if re.search(
        r"^(how do i feel|what am i feeling|how am i feeling|how am i doing emotionally|what is my emotional state)[\s.!?]*$",
        qn,
    ):
    ...
    if re.search(
        r"\b(bias|biased|unbiased|neutral|objective|balanced|fair|trustworthy|credible|reliable|factual|propaganda|slant|partisan)\b",
        qn,
    ):
    ...
    if (
        ...
        and not re.search(
            r"\b(amplifier|plate|screen|anode|cathode|grid|bias|class a|class ab1|voltage|current|load|impedance|operating point|quiescent)\b",
            qn,
        )
    ):
    ...
    if (
        re.search(r"\b807s?\b", qn)
        and re.search(r"\b(pair|two|2)\b", qn)
        and re.search(r"\b(push[ -]?pull|pp)\b", qn)
        and re.search(r"\b(class )?ab1\b", qn)
        and re.search(r"\b(power|output|watt|watts)\b", qn)
    ):
    ...
    if re.search(r"\b(facts|assumptions|external dependencies)\b", qn) and re.search(
        r"\bis water wet\b", qn
    ):
        return "water_wet_structured"
    if re.search(r"(^|[^a-z0-9_])(dog|dogs|pet|puppy)([^a-z0-9_]|$)", qn) and re.search(
        r"(^|[^a-z0-9_])(rocket|rockets|attack|attacks|explosion|explosions|blast|blasts|bomb|bombing|siren|sirens|firework|fireworks|shelling|missile|missiles)([^a-z0-9_]|$)",
        qn,
    ):
