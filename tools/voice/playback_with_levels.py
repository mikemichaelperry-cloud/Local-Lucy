...
def play_wav_file_with_levels(
    ...
    *,
    ...
    writer_thread = threading.Thread(
        target=_write_output_level, args=(current_level, levels_file, running), daemon=True
    )
    updater_thread = threading.Thread(target=level_updater, daemon=True)
    ...
    try:
        ...
        if proc.returncode != 0:
            stderr_text = (
                stderr_data.decode("utf-8", errors="replace").strip() if stderr_data else ""
            )
            raise PlaybackError(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        levels_file = (
            Path(sys.argv[2]) if len(sys.argv) >= 3 else Path("/tmp/test_audio_levels.json")
        )
        try:
