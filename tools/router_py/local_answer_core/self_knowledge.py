...
# model has an 8192-token context window and long system blocks get ignored.
...
_SELF_KNOWLEDGE_TEMPLATE = (
    ...
    "memory-first continuation handling with full truncated-output storage; model-aware prompt shaping.\n"
    "Routing is automatic: LOCAL (default), AUGMENTED (Wikipedia, Wikivoyage, and trusted tourism-board evidence "
    "with optional synthesis by OpenAI/Kimi), NEWS, TIME, WEATHER, FINANCE, EVIDENCE, SELF_REVIEW (Engineering mode). "
    "Do not ask the user to pick a mode.\n"
    "Capabilities: coding, writing, reasoning, voice, tube database (648 types), "
    "live data via AUGMENTED/NEWS/WEATHER, travel/destination answers from trusted sources, "
    "and Engineering-mode read-only review of local files (up to ~200,000 characters / ~1,500 lines of code or text).\n"
    ...
    "cannot read files on the computer in normal chat mode. "
    "Engineering mode (self-analysis mode) can read local files for review when explicitly enabled in the HMI.\n"
    ...
    "describe the {version_label} stack above, including the Gemma 4 / Llama 3.1 8B model choice and memory-first continuity. "
    "If asked about reading local files, explain that normal chat mode cannot access files directly, "
    "but Engineering mode (self-analysis mode) can read local files for review when enabled, with a context budget of up to ~200,000 characters (~1,500 lines). "
    "If asked about fallback providers or evidence sources (e.g., OpenAI, Kimi, Wikipedia, Wikivoyage, "
    "Israel Ministry of Tourism / Go Israel), say that Wikipedia and Wikivoyage are primary augmented sources, "
    "OpenAI/Kimi may synthesize evidence when the router activates AUGMENTED mode, "
    "Israel travel queries can use Ministry of Tourism / Go Israel data, and your default knowledge source is the local "
...
_MODEL_IDENTITIES: dict[str, tuple[str, str]] = {
    ...
    # Context figures are the deployed windows (PARAMETER num_ctx in the
    # matching config/Modelfile.*), not the models' architectural ceilings.
    "local-lucy-llama31": ("llama3.1:8b", "~8B parameters, 8192-token context"),
    "local-lucy-llama31:latest": ("llama3.1:8b", "~8B parameters, 8192-token context"),
    "local-lucy-gemma4": (
        "gemma4:12b-it-qat",
        "~12B parameters, 8192-token context (128k architectural ceiling)",
    ),
    ...
    "gemma4_code_review_agentic": (
        ...
        "~12B parameters, 16384-token context (128k architectural ceiling), code-review specialist",
    ),
}
