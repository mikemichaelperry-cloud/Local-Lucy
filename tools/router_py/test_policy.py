...
class TestNormalizeAugmentationPolicy(unittest.TestCase):
    ...
    def test_direct_allowed_variants(self):
        ...
        direct_variants = ["direct_allowed", "DIRECT_ALLOWED", "direct", "DIRECT", "Direct", "2"]
    ...
    def test_unknown_defaults_to_disabled(self):
        ...
        unknown_variants = ["invalid", "unknown", "random_string", "enabled", "auto", "maybe"]
    ...
    def test_family_with_symptoms_triggers_medical(self):
        ...
        for query in medical_queries:
            with self.subTest(query=query):
                ...
                self.assertTrue(
                    reason.startswith("medical"), f"Expected medical reason, got: {reason}"
                )
...
class TestManifestEvidenceSelectionLabel(unittest.TestCase):
    ...
    def test_no_evidence_mode(self):
        ...
        self.assertEqual(manifest_evidence_selection_label(None, None), "not_applicable")
        self.assertEqual(manifest_evidence_selection_label("", "reason"), "not_applicable")

    def test_default_light(self):
        """Test default light reason."""
        self.assertEqual(
            manifest_evidence_selection_label("light", "default_light"), "default-light"
        )
        self.assertEqual(manifest_evidence_selection_label("light", ""), "default-light")
    ...
    def test_policy_triggered(self):
        ...
        self.assertEqual(
            manifest_evidence_selection_label("required", "policy_medical"), "policy-triggered"
        )
        self.assertEqual(
            manifest_evidence_selection_label("required", "medical_context"), "policy-triggered"
        )
        self.assertEqual(
            manifest_evidence_selection_label("required", "conflict_live"), "policy-triggered"
        )

    def test_manifest_selected(self):
        """Test manifest selected fallback."""
        self.assertEqual(
            manifest_evidence_selection_label("required", "unknown_reason"), "manifest-selected"
        )
        self.assertEqual(
            manifest_evidence_selection_label("required", "calculated"), "manifest-selected"
        )
