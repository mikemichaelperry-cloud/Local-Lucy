...
def build_record_command(
    recorder_bin: str, channels: int = 1, sample_rate: int = 16000
) -> list[str]:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
            ...
            with wave.open(tmp.name, "wb") as wav_file:
                ...
                max_dur = (
                    ...
                    else _MAX_RECORDING_DURATION_SECONDS
                )
                ...
                while _recording and not check_stop_signal(stop_file):
                    if time.monotonic() - recording_start_time >= max_dur:
                        logger.warning(
                            f"Recording stopped after reaching max duration ({max_dur}s)"
                        )
                        break
                    try:
...
def main() -> int:
    ...
    parser.add_argument("--output", "-o", type=Path, required=True, help="Output WAV file path")
    parser.add_argument(
        "--runtime-file",
        "-r",
        type=Path,
