...
class TestSQLiteWrites:
    ...
    def test_sqlite_outcome_includes_memory_telemetry(
        self, writer, mock_state_manager, sample_route, sample_context
    ):
    ...
    def test_sqlite_failure_logs_error(
        self, writer, mock_state_manager, sample_route, sample_result, sample_context, caplog
    ):
    ...
    def test_sqlite_disabled_skips_write(
        self, mock_state_manager, mock_logger, sample_route, sample_result, sample_context
    ):
        writer = StateWriter(
            state_dir=Path("/tmp"),
            state_manager=mock_state_manager,
...
class TestTerminalOutcome:
    ...
    def test_terminal_outcome_error(self, writer, mock_state_manager):
        writer.record_terminal_outcome(
            "execution_error", "LOCAL", error_msg="boom", execution_time_ms=10
        )
        args, _ = mock_state_manager.write_outcome.call_args
...
class TestPIIRedaction:
    ...
    def test_redact_email_in_json_payload(
        self, writer, sample_route, sample_result, tmp_path, monkeypatch
    ):
    ...
    def test_redact_phone_in_json_payload(
        self, writer, sample_route, sample_result, tmp_path, monkeypatch
    ):
        """PII in question must be redacted in JSON payload request_text."""
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
    ...
    def test_redact_ssn_in_json_payload(
        self, writer, sample_route, sample_result, tmp_path, monkeypatch
    ):
        """PII in question must be redacted in JSON payload request_text."""
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
    ...
    def test_no_redaction_for_clean_text_in_json(
        self, writer, sample_route, sample_result, tmp_path, monkeypatch
    ):
    ...
    def test_redact_error_message_in_sqlite(self, writer, mock_state_manager):
        ...
        result = ExecutionResult(
            ...
            provider_usage_class="local",
            response_text="",
            error_message="Failed for user@corp.org",
...
class TestJsonStateFiles:
    ...
    def test_write_json_creates_result_file(
    ...
    def test_write_json_creates_route_file(
        self, writer, sample_route, sample_result, sample_context, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
        writer.write_json_state_files(sample_route, sample_result, sample_context)
    ...
    def test_write_json_creates_history_file(
        self, writer, sample_route, sample_result, sample_context, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
        writer.write_json_state_files(sample_route, sample_result, sample_context)
    ...
    def test_route_snapshot_schema(
        self, writer, sample_route, sample_result, sample_context, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
        writer.write_json_state_files(sample_route, sample_result, sample_context)
    ...
    def test_history_entry_schema(
        ...
        writer.write_json_state_files(sample_route, sample_result, sample_context)
        lines = (
            (tmp_path / "request_history.jsonl").read_text(encoding="utf-8").strip().splitlines()
        )
        assert len(lines) == 1
    ...
    def test_history_appends_each_write(
        self, writer, sample_route, sample_result, sample_context, tmp_path, monkeypatch
    ):
    ...
    def test_trusted_evidence_metadata_persisted(
        self, writer, sample_context, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
        route = RoutingDecision(
            route="EVIDENCE",
        ...
        result = ExecutionResult(
            ...
            provider="trusted",
            provider_usage_class="local",
            response_text="Fallback from trusted domains.",
    ...
    def test_control_state_from_env(
        self, writer, sample_route, sample_result, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("LUCY_UI_STATE_DIR", str(tmp_path))
    ...
    def test_no_exception_on_failure(
        self, writer, sample_route, sample_result, sample_context, tmp_path, monkeypatch, caplog
    ):
    ...
    def test_request_id_propagation(
        ...
        mock_sm = MagicMock()
        writer_sqlite = StateWriter(
            "default", state_manager=mock_sm, logger=logging.getLogger("test")
        )
