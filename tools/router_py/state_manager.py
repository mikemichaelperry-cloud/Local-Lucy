...
class StateManager:
    ...
    def _ensure_namespace(self) -> int:
        ...
        with self._transaction() as conn:
            cursor = conn.execute(
                "INSERT OR IGNORE INTO namespaces (name) VALUES (?)", (self.namespace,)
            )
            cursor = conn.execute("SELECT id FROM namespaces WHERE name = ?", (self.namespace,))
            row = cursor.fetchone()
            namespace_id = row[0]
...
if __name__ == "__main__":
    ...
    result = sm.write_outcome(
        {"success": True, "duration_ms": 150, "result": {"items_found": 5, "query_time": 45}}
    )
    print(f"   Write result: {result}")
    ...
    session_result = sm.write_session(
        "test_session", {"user": "test", "data": [1, 2, 3]}, ttl_seconds=300
    )
