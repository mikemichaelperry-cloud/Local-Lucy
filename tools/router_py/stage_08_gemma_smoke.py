...
Runs a small set of core requests through the HMI surface using Gemma as the
local model, verifies that Llama is never resident at the same time, and
records route/outcome/response metadata.
...
Requires:
    - Ollama running with local-lucy-gemma4:latest available.
...
def _load_gemma_exclusively() -> None:
    """Unload every Lucy model, then load Gemma and confirm Llama is gone."""
    unload_all_lucy_models()
    for m in _lucy_models_loaded():
        if not _wait_for_unload(m):
            raise RuntimeError(f"Could not unload {m} before Gemma smoke")
    ...
    if any(LLAMA_MODEL in m for m in loaded):
        raise RuntimeError(f"Llama {LLAMA_MODEL} is still loaded alongside Gemma")
...
def _set_state_model_to_gemma() -> None:
    """Update current_state.json so the HMI surface uses Gemma."""
...
def main() -> int:
    ...
    # Pin Gemma for this smoke run in both env and state file.
    ...
    try:
        ...
        for case in cases:
            ...
            if any(LLAMA_MODEL in m for m in loaded):
                case_passed = False
                outcome["parallel_model_error"] = f"Llama loaded alongside Gemma: {loaded}"
    ...
    report_path = Path("qualification/results/stage_08_gemma_smoke.json")
