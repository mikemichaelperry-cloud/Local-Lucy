        ...
        with patch("router_py.request_pipeline.process") as mock_process:
            mock_process.return_value = (
                _make_outcome(
                    ...
                    metadata={
                        "voice_text": "Headline one, from Source A. Headline two, from Source B."
                    },
                ),
                None,
                None,
            ...
            assert (
                outcome.metadata.get("voice_text")
                == "Headline one, from Source A. Headline two, from Source B."
            )
...
class TestVoiceHmiBridge:
    ...
    def _setup_env(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCY_UI_ROOT", f"{project_root}/ui-v10")
        monkeypatch.setenv("LUCY_RUNTIME_NAMESPACE_ROOT", project_root)
        # Add ui-v10 to path for imports (so 'app.services.runtime_bridge' resolves)
        ui_root = f"{project_root}/ui-v10"
...
class TestKokoroWorkerMock:
    ...
    def test_kokoro_worker_request_format(self):
        ...
        assert (
            serialized
            == '{"cmd": "synthesize", "engine": "kokoro", "text": "Hello", "voice": "af_bella"}'
        )
