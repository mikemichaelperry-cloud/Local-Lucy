...
from backend.policy import requires_evidence_mode
...
# Import direct Python providers (eliminates subprocess)
try:
    from backend.providers import OpenAIProvider, KimiProvider, WikipediaProvider, ProviderResult
    HAS_DIRECT_PROVIDERS = True
except ImportError:
    HAS_DIRECT_PROVIDERS = False

# Import news provider for live news fetching
try:
    from backend.news_provider import NewsProvider, NewsResult
...
def _load_session_memory_context() -> str:
    ...
    Returns:
        ...
        or empty string if memory is disabled or file doesn't exist.
    """
    # Check if memory is enabled
    session_mem = os.environ.get("LUCY_SESSION_MEMORY", "NOT_SET")
    _debug_log(f"_load_session_memory_context: LUCY_SESSION_MEMORY={session_mem}")
    ...
    _debug_log(f"Looking for memory file: {mem_path}")
    ...
    try:
        ...
        _debug_log(f"Could not read memory file: {e}")
        return ""
    ...
    _debug_log(f"Loaded {len(context)} chars of context")
    return context
...
them according to the route type (bypass, provisional, or full). It handles:
    ...
    def execute(
        ...
        if use_python_path and route.route in ("FULL", "EVIDENCE", "NEWS", "AUGMENTED", "LOCAL"):
    ...
    async def execute_async(
        ...
        try:
            ...
            if route.route in ("FULL", "EVIDENCE", "NEWS", "AUGMENTED"):
    ...
    async def _execute_full_route_python(
        ...
        evidence = None
        if route.route in ("EVIDENCE", "NEWS", "FULL", "AUGMENTED"):
            evidence = await self._fetch_evidence(question, route)
        ...
        if route.provider == "local":
            response = await self._call_local_model_async(prompt, context)
    ...
    async def _fetch_evidence(
        ...
        """
        self._logger.info(f"Fetching evidence for route={route.route}, provider={route.provider}")
        
        # For NEWS route, fetch live news from RSS/API sources
        if route.route == "NEWS":
            return await self._fetch_news_evidence(question)
    ...
    async def _fetch_news_evidence(self, question: str) -> dict[str, Any] | None:
        """
        Fetch live news from RSS feeds and news APIs.
        ...
        self._logger.info(f"Fetching live news for: {question[:50]}...")
        
        try:
            # Run news fetch in thread pool to not block event loop
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                None, NewsProvider.fetch_news, question
            )
    ...
    async def _call_local_model_async(
        ...
        try:
            result = await answer.generate_answer(prompt)
    ...
    def _call_openai_for_response(self, prompt: str) -> str:
        ...
        # Use direct Python provider if available (no subprocess)
        if HAS_DIRECT_PROVIDERS:
            try:
                result = OpenAIProvider.call(prompt)
                if result.ok:
                    return result.text
    ...
    def _call_kimi_for_response(self, prompt: str) -> str:
        ...
        if HAS_DIRECT_PROVIDERS:
            try:
                result = KimiProvider.call(prompt)
                if result.ok:
                    return result.text
                return f"Error: {result.error}"
            except Exception as e:
    ...
    def _call_local_worker(
        ...
        Returns:
            CompletedProcess with stdout, stderr, and returncode
        """
        # Load session memory context if enabled and not already provided
        if not env.get("LUCY_SESSION_MEMORY_CONTEXT"):
            session_memory = _load_session_memory_context()
            _debug_log(f"_call_local_worker: Loaded {len(session_memory)} chars of session memory")
            if session_memory:
                env["LUCY_SESSION_MEMORY_CONTEXT"] = session_memory
                self._logger.debug(f"Loaded session memory context ({len(session_memory)} chars)")
        ...
        try:
            ...
            if result.returncode != 0:
                return ExecutionResult(
                    ...
                    provider=provider,
                    provider_usage_class=self._provider_usage_class_for(provider),
                    error_message=result.stderr or "Provider dispatch failed",
