...
def select_route(
    ...
    # Local model may refuse with "evidence required" which triggers fallback
    if classification.intent_family == "current_evidence":
        # News-specific queries should use NEWS route for RSS feed fetching
        # This takes priority over evidence_mode to ensure RSS news is used
        if classification.category in ("news_world", "news_israel", "news_australia"):
            return _make_news_decision(classification)
        
        # Current facts with temporal signals (e.g., "Who won the election?")
        # should use augmented for up-to-date information
        if classification.category == "current_fact" and classification.confidence > 0.7:
            # Go directly to augmented for high-confidence current facts
            # Local model won't have up-to-date information for these
    ...
    # Current evidence without specific category handling
    if classification.intent_family == "current_evidence":
