#!/usr/bin/env python3
"""Offscreen HMI test: Whisper GPU succeeds on first try."""
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="whisper_gpu_ok_ui_") as tmp_dir:
        sandbox = Sandbox(Path(tmp_dir))
        sandbox.prepare_import_environment()

        import sys
        print("DEBUG: LUCY_RUNTIME_NAMESPACE_ROOT =", os.environ.get("LUCY_RUNTIME_NAMESPACE_ROOT"))
        print("DEBUG: state_store already imported?", "app.services.state_store" in sys.modules)

        from PySide6.QtWidgets import QApplication
        from app.main_window import OperatorConsoleWindow

        from app.services import state_store
        print("DEBUG: state_store.STATE_DIRECTORY =", state_store.STATE_DIRECTORY)
        ...
        print("DEBUG: status =", window._latest_state_snapshot.voice_runtime.get("status"))
        print("DEBUG: vr file =", window._latest_state_snapshot.voice_runtime)
        print("DEBUG: ui events =", window._ui_event_lines)
        print("DEBUG: voice_runtime_path exists =", sandbox.voice_runtime_path.exists())
        assert_ok(
        ...
        wait_for(lambda: not window._voice_action_in_flight, app, 10.0, "voice stop gpu success")
        assert_ok(
            window._latest_state_snapshot.voice_runtime["status"] == "idle",
            "release should return to idle after GPU success",
        )

        # Verify STT backend metadata shows GPU
        vr = window._latest_state_snapshot.voice_runtime
        assert_ok(vr.get("stt_backend") == "gpu", f"stt_backend should be 'gpu', got {vr.get('stt_backend')!r}")
        assert_ok(
            not vr.get("stt_fallback_reason"),
            f"stt_fallback_reason should be empty, got {vr.get('stt_fallback_reason')!r}",
        )

        # Verify HMI shows GPU indicator
        stt_text = window.control_panel._voice_stt_label.text()
        assert_ok("STT: GPU" in stt_text, f"HMI should show STT: GPU, got {stt_text!r}")
        assert_ok("→" not in stt_text, f"HMI should not show fallback arrow, got {stt_text!r}")
        ...
        assert_ok(
            ...
            "voice submit should persist transcript on GPU path",
        )

        close_window(app, window)
        print("WHISPER_GPU_SUCCESS_OFFSCREEN_OK")
        return 0
...
class Sandbox:
    def __init__(self, root: Path) -> None:
        ...
        self.transcript_text = "hello from gpu success offscreen test"
        self.response_text = "mock response for gpu success test"
    ...
    def _install_common_binaries(self) -> None:
        ...
        # Mock whisper: always succeeds (simulates GPU working)
        write_executable(
