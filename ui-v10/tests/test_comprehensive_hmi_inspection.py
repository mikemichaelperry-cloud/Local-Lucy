...
def inspect_level(window: MainWindow, level: str) -> None:
    ...
    record("control_panel", level, "augmentation_policy exists", cp._augmentation_policy_selector is not None)
    record("control_panel", level, "provider_selector exists", cp._augmented_provider_selector is not None)
    ...
    record("control_panel", level, "profile_group visible at engineering",
    ...
    for key in ("Current Route", "Source Type", "Conversation", "Voice State",
                "Health", "Model", "Augmented Policy", "Configured Provider",
                "Configured Provider Paid", "Last Request Provider",
                "Last Request Paid", "Session Augmented Calls",
                "Session Paid Augmented Calls"):
        exists = key in summary
        text = summary[key].text() if exists else "MISSING"
        record("status_summary", level, f"label '{key}'",
               exists, f"text={text!r}")
    ...
    for key in ("Voice Backend", "Voice Error", "GPU Acceleration"):
        ...
        record("status_details", level, f"label '{key}'",
               exists, f"text={text!r}")

    # Request summary labels
    req_summary = sp._request_summary_labels
    for key in ("Status", "Completed At", "Answer Path", "Trust",
                "Augmented", "Route Mode", "Outcome Code"):
        exists = key in req_summary
        text = req_summary[key].text() if exists else "MISSING"
        record("request_summary", level, f"label '{key}'",
               exists, f"text={text!r}")

    # Request detail labels
    req_detail = sp._request_detail_labels
    for key in ("Request ID", "Request Text", "Error", "Route Reason",
                "Route Confidence", "Operator Note", "Action Hint",
                "Verification Status", "Estimated Confidence", "Source Basis",
                "Augmented Direct Request", "Augmented Provider Status",
                "Evidence Created", "Primary Outcome", "Recovery Lane",
                "Control State"):
        exists = key in req_detail
        text = req_detail[key].text() if exists else "MISSING"
        record("request_detail", level, f"label '{key}'",
               exists, f"text={text!r}")

    # Event log panel visibility
    event_visible = not window.event_log_panel.isHidden()
    record("layout", level, "event_log_panel visible at engineering",
    ...
    record("decision_trace", level, "decision_trace button visible",
           dt_visible, f"visible={dt_visible}")
    ...
    if freshness:
        record("status_panel", level, "freshness_label has text",
               len(freshness.text()) > 0, f"text={freshness.text()!r}")
    ...
    if level == "engineering":
        ...
        record("engineering_views", level, "history_maintenance_view present", hist_maintenance is not None)
    else:
        # isHidden() only checks explicit hide; isVisible() accounts for parent visibility
        record("engineering_views", level, "advanced views hidden at operator",
               (adv_state is None or not adv_state.isVisible()) and
               (adv_request is None or not adv_request.isVisible()) and
               (hist_maintenance is None or not hist_maintenance.isVisible()))


def inspect_route_payload(window: MainWindow, level: str, payload: dict, expected_provider: str, expected_paid: str) -> None:
    ...
    record("route_accuracy", level, f"provider for {payload['route']['mode']}",
    ...
    record("route_accuracy", level, f"paid flag for {payload['route']['mode']}",
    ...
    record("route_accuracy", level, f"current route reflects {payload['route']['mode']}",
...
def main() -> int:
    ...
    try:
        ...
        inspect_route_payload(
            window, "engineering",
            make_payload(route_mode="LOCAL", provider="none", paid=False,
                         request_id="req-test-local"),
            expected_provider="none", expected_paid="no",
        )

        # AUGMENTED route (OpenAI, paid)
        inspect_route_payload(
            window, "engineering",
            make_payload(route_mode="AUGMENTED", provider="openai", paid=True,
                          augmented_direct=True, trust_class="unverified",
                          request_id="req-test-aug-openai"),
            expected_provider="openai", expected_paid="yes",
        )

        # AUGMENTED route (Wikipedia, unpaid)
        inspect_route_payload(
            window, "engineering",
            make_payload(route_mode="AUGMENTED", provider="wikipedia", paid=False,
                          trust_class="evidence_backed",
                          request_id="req-test-aug-wiki"),
            expected_provider="wikipedia", expected_paid="no",
        )

        # NEWS route
        inspect_route_payload(
            window, "engineering",
            make_payload(route_mode="NEWS", provider="none", paid=False,
        ...
        inspect_route_payload(
            window, "engineering",
            make_payload(route_mode="TIME", provider="none", paid=False,
                         request_id="req-test-time"),
            expected_provider="none", expected_paid="no",
        )
        ...
        cp = window.control_panel
        record("control_state", "engineering", "mode_selector default value",
               cp._mode_selector.currentText() in {"auto", "online", "offline"},
               f"value={cp._mode_selector.currentText()!r}")
        record("control_state", "engineering", "memory_selector has on/off",
        ...
        record("conversation", "engineering", "history widget has content after injection",
               len(history_html) > 100,
               f"html_length={len(history_html)}")
        record("conversation", "engineering", "history contains 'test answer'",
        ...
        if dt_button and not dt_button.isHidden():
            ...
            record("decision_trace", "engineering", "decision_trace button has text",
                   len(dt_text) > 0, f"text={dt_text!r}")
            # Toggle it on
            ...
            if dt_panel:
                record("decision_trace", "engineering", "decision_trace panel visible after toggle",
                ...
                record("decision_trace", "engineering", "decision_trace panel has content",
                       has_content, f"text_length={len(trace_view.toPlainText()) if trace_view else 0}")
            else:
                record("decision_trace", "engineering", "decision_trace panel exists after toggle",
                       False, "_decision_trace_panel not found on window")
        else:
            record("decision_trace", "engineering", "decision_trace button accessible",
                   False, "button hidden or missing")
        ...
        if hasattr(sp, "_advanced_state_view") and sp._advanced_state_view:
            ...
            record("advanced_views", "engineering", "advanced_state_view has JSON content",
                   len(text) > 50, f"length={len(text)}")
        else:
            record("advanced_views", "engineering", "advanced_state_view present",
                   False, "widget missing")

        # --- Phase 7: Event log ---
        print("\n--- Event Log ---")
        elp = window.event_log_panel
        record("event_log", "engineering", "event_log_panel visible at engineering",
               not elp.isHidden())
        log_text = elp.toPlainText() if hasattr(elp, "toPlainText") else ""
        record("event_log", "engineering", "event_log has content or is ready",
               True, f"length={len(log_text)}")
    ...
    if failed:
        ...
        for r in REPORT:
            if not r["ok"]:
                print(f"  ❌ [{r['category']}] level={r['level']} check={r['check']}: {r['detail']}")
    else:
        print("\n✅ ALL CHECKS PASSED")
