...
class RoutingTestLogger:
    ...
    def test_result(
        self, test_name: str, passed: bool, expected: str, actual: str, details: str = ""
    ) -> None:
        """Log test result."""
...
def get_current_state() -> dict[str, Any]:
    ...
    state_file = (
        Path.home() / ".codex-api-home" / "lucy" / "runtime-v10" / "state" / "current_state.json"
    )
    try:
...
def run_mode_routing_tests(logger: RoutingTestLogger) -> None:
    ...
    for test in tests:
        ...
        if isinstance(expected, list):
            passed = actual_mode in expected or any(
                e.lower() in actual_mode.lower() for e in expected
            )
...
def run_evidence_toggle_tests(logger: RoutingTestLogger) -> None:
    ...
    passed = (
        actual_mode in ["EVIDENCE", "AUGMENTED", "LOCAL", "TIME"]
        or "evidence" in actual_mode.lower()
    )
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["all", "routing", "memory", "evidence", "conversation"],
        default="all",
