...
def _clear_python_caches():
    ...
    router_py_path = lucy_base / "tools" / "router_py"
...
from app.backend import ensure_control_env, execute_plan_python
...
class TestReporter:
    ...
    def pass_(self, test: str, details: str = "") -> None:
        ...
        if details:
            print(f"      {details}")
        self.results.append(
            {"test": test, "status": "PASS", "details": details, "time": datetime.now().isoformat()}
        )
    ...
    def warn(self, test: str, msg: str) -> None:
        ...
        print(f"      {msg}")
        self.results.append(
            {"test": test, "status": "WARN", "details": msg, "time": datetime.now().isoformat()}
        )
...
def get_last_route() -> dict:
    ...
    try:
        path = (
            Path.home() / ".codex-api-home" / "lucy" / "runtime-v10" / "state" / "last_route.json"
        )
        if path.exists():
            with open(path) as f:
                return json.load(f)
...
def test_mode_routing(r: TestReporter) -> None:
    ...
    tests = [
        ...
        (
            "Identity → LOCAL/POLICY",
            "Who are you?",
            ["LOCAL", "POLICY", "IDENTITY"],
            "Identity handled locally",
        ),
        (
            "Current Info → TIME",
            "What time is it in Tokyo?",
            ["TIME"],
            "Time queries route to time API",
        ),
        (
            "Wikipedia Search → LOCAL/AUGMENTED",
            "Search Wikipedia for Albert Einstein",
            ["LOCAL", "AUGMENTED", "EVIDENCE"],
            "May route local depending on classification",
        ),
        (
            "Vague Query → CLARIFY/LOCAL",
            "Tell me about it",
            ["CLARIFY", "LOCAL"],
            "Vague queries may need clarification",
        ),
    ]
...
def test_state_consistency(r: TestReporter) -> None:
    ...
    if os.environ.get("LUCY_SESSION_MEMORY") == "1":
        ...
        r.fail(
            ...
            os.environ.get("LUCY_SESSION_MEMORY", "missing"),
        )
...
def main():
    ...
    default_output = (
        ...
        / "runtime-v10"
        / "logs"
        / "routing_verification_report.json"
    )
