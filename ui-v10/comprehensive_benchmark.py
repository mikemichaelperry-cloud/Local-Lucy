...
def check_prerequisites():
    ...
    # Check TTS engines
    result = run_command(
        f"cd {SNAPSHOT_ROOT} && python3 -c \"import sys; sys.path.insert(0, 'tools'); from voice.backends import kokoro_backend; print(kokoro_backend.detect_binary({SNAPSHOT_ROOT!r}) is not None)\""
    )
    checks["tts_kokoro"] = "True" in result.get("stdout", "")

    # Check if Kokoro worker is running
    result = run_command(
        "pgrep -f kokoro_session_worker >/dev/null && echo 'running' || echo 'stopped'"
    )
...
def benchmark_text_latency():
    ...
    for prompt in prompts:
        ...
        for run in range(1, 3):
            start = time.time()
            result = run_command(f"python3 {tool} submit --text '{prompt}'", timeout=130)
...
def benchmark_tts():
    ...
    result = run_command(f"cd {SNAPSHOT_ROOT} && python3 {tool} --runs 2", timeout=180)

    if result["success"]:
        try:
            # Parse JSON output
            data = json.loads(result["stdout"])
            return {"variants_tested": data.get("variants", []), "summary": data.get("summary", {})}
...
def main():
    ...
    print("=" * 60)
    print(f"  1. Launch HMI: cd {UI_ROOT} && source .venv/bin/activate && python app/main.py")
