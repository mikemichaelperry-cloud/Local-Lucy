...
- Add a `route == "SELF_REVIEW"` branch in `_set_generation_profile()` that returns a `("self_review", self_review_max_tokens, "- Provide a thorough, broad, balanced code review covering architecture, maintainability, readability, safety, and testability. Address every listed hotspot explicitly. Do not fixate on a single issue or function.")` profile.
