...
| 3 | Warn when SELF_REVIEW prompt may exceed model context | Minor | Whole-branch review | Default `self_review_context_chars=200000` assumes a model with at least ~64k context (e.g., Gemma 4). Emit a `logger.warning` when the estimated prompt tokens approach the active model's `num_ctx`. |
